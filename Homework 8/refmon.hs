...
leastUpperBound n1 n2 tree = do
  ...
  -- (uses Maybe as monad here by calling ancestors which calls parent which uses Maybe as a monad)
  a <- (ancestors n1 tree)
  b <- (ancestors n2 tree)
...
trib :: Int -> Int
trib n = let initialState = (1, 1, 1)
             comp          = tribM n
         in case runState comp initialState of
...
simpleBalancedHelper :: String -> [Char] -> Bool
simpleBalancedHelper "" stack = null stack
simpleBalancedHelper (p:ps) stack
        | p == '(' = simpleBalancedHelper ps (p:stack) 
        | p == ')' && (head stack /= '(' || null stack) = False
        | p == '[' = simpleBalancedHelper ps (p:stack)
        | p == ']' && (head stack /= '[' || null stack) = False
        | p == '{' = simpleBalancedHelper ps (p:stack)
        | p == '}' && (head stack /= '{' || null stack) = False
        | p == '<' = simpleBalancedHelper ps (p:stack)
        | p == '>' && (head stack /= '<' || null stack) = False
        | otherwise = simpleBalancedHelper ps (tail stack)
...
counter :: Int -> Int --counter for mersennes
counter n = do 
  return n + 1
